...
const PanoramaManagerPage = () => {
    ...
    const handleCreateScene = async (e) => {
        ...
        try {
            ...
            setNewScene({ title: '', image: null, is_start_scene: false, sort_order: scenes.length + 2, is_active: true });
            loadData();
        } catch (error) {
    ...
    return (
        <div>
            {successMessage && (
                <div style={{
                    position: 'fixed', top: '24px', right: '24px', backgroundColor: '#10b981', color: '#fff',
                    padding: '16px 24px', borderRadius: theme.radius.md, zIndex: 9999, display: 'flex', gap: '8px'
                }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: theme.spacing.md, marginBottom: theme.spacing.lg }}>
                <button
                    ...
                    style={{ padding: theme.spacing.sm, border: `1px solid ${theme.colors.border}`, borderRadius: theme.radius.sm, display: 'flex', alignItems: 'center', gap: '4px', cursor: 'pointer', background: '#fff' }}
                >
                    <ArrowLeft size={20} /> Back
                </button>
                <h1 style={{ fontSize: '28px', fontWeight: 'bold', margin: 0 }}>Panoramas: {building?.name}</h1>
            </div>

            <div style={{ display: 'grid', gridTemplateColumns: '300px 1fr 400px', gap: theme.spacing.lg, height: 'calc(100vh - 200px)' }}>
                {/* Scenes List */}
                <ScenesList 
                    scenes={scenes}
...
const ScenesList = ({ scenes, selectedScene, setSelectedScene, onDeleteScene, newScene, setNewScene, onCreateScene, saving, errors }) => (
    <div style={{ backgroundColor: '#fff', borderRadius: theme.radius.md, padding: theme.spacing.md, overflowY: 'auto' }}>
        <h2 style={{ fontSize: '16px', fontWeight: '600', marginBottom: theme.spacing.md }}>Scenes</h2>
            ...
            {scenes.map(scene => (
                <div
                    ...
                    style={{
                        ...
                        cursor: 'pointer',
                        backgroundColor: selectedScene?.id === scene.id ? 'rgba(138, 21, 56, 0.05)' : '#fff',
                        display: 'flex',
                ...
                >
                    <div>
                        ...
                        {scene.is_start_scene && (
                            <span style={{ fontSize: '12px', color: theme.colors.primary, display: 'flex', alignItems: 'center', gap: '4px', marginTop: '2px' }}>
                    ...
                    <button
                        ...
                        style={{ color: theme.colors.error, background: 'none', border: 'none', cursor: 'pointer', padding: '4px' }}
                    >
                        <Trash2 size={16} />
                    </button>
                </div>
        ...
        <div style={{ borderTop: `1px solid ${theme.colors.border}`, paddingTop: theme.spacing.md }}>
            <h3 style={{ fontSize: '14px', fontWeight: '600', marginBottom: theme.spacing.sm }}>New Scene</h3>
            <form onSubmit={onCreateScene}>
                <input
                    ...
                    style={{ width: '100%', padding: '8px', marginBottom: '8px', borderRadius: '4px', border: `1px solid ${errors.title ? 'red' : '#ccc'}` }}
                />
                ...
                <label style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '12px', marginBottom: '8px' }}>
                    <input
                        ...
                        onChange={(e) => setNewScene({ ...newScene, is_start_scene: e.target.checked })}
                    />
                    Start Scene
                </label>
                <button
                    type="submit"
                    disabled={saving}
                    style={{ width: '100%', padding: '8px', background: theme.colors.primary, color: 'white', border: 'none', borderRadius: '4px', cursor: saving ? 'not-allowed' : 'pointer', fontSize: '14px', fontWeight: '600' }}
                >
...
const ScenePreview = ({ selectedScene }) => (
    <div style={{ backgroundColor: '#fff', borderRadius: theme.radius.md, padding: theme.spacing.md, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
        {selectedScene ? (
            <>
                <h2 style={{ fontSize: '18px', fontWeight: '600', marginBottom: theme.spacing.md }}>{selectedScene.title}</h2>
                <img
                    src={selectedScene.image_url}
                    alt={selectedScene.title}
                    style={{ width: '100%', maxHeight: '400px', objectFit: 'contain', borderRadius: theme.radius.sm }}
                />
            </>
        ) : (
...
const HotspotsPanel = ({ hotspots, scenes, selectedScene, newHotspot, setNewHotspot, editingHotspot, setEditingHotspot, onCreateHotspot, onUpdateHotspot, onDeleteHotspot }) => (
    <div style={{ backgroundColor: '#fff', borderRadius: theme.radius.md, padding: theme.spacing.md, overflowY: 'auto' }}>
        <h2 style={{ fontSize: '16px', fontWeight: '600', marginBottom: theme.spacing.md }}>Hotspots</h2>
        
        {!selectedScene ? (
            <p style={{ fontSize: '14px', color: theme.colors.text.secondary }}>Select a scene to manage hotspots</p>
        ) : (
            <>
                <div style={{ marginBottom: theme.spacing.md }}>
                    {hotspots.map(hotspot => (
                        <div key={hotspot.id} style={{ padding: theme.spacing.sm, border: `1px solid ${theme.colors.border}`, borderRadius: theme.radius.sm, marginBottom: theme.spacing.xs }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start' }}>
                                <div style={{ flex: 1 }}>
                                    <div style={{ fontSize: '14px', fontWeight: '500' }}>{hotspot.label}</div>
                                    <div style={{ fontSize: '12px', color: theme.colors.text.secondary }}>→ {hotspot.target_scene_title}</div>
                ...
                <div style={{ borderTop: `1px solid ${theme.colors.border}`, paddingTop: theme.spacing.md }}>
                    <h3 style={{ fontSize: '14px', fontWeight: '600', marginBottom: theme.spacing.sm }}>New Hotspot</h3>
                    <form onSubmit={onCreateHotspot}>
                        ...
                        <div style={{ marginBottom: '8px' }}>
                            <label style={{ display: 'block', fontSize: '12px', marginBottom: '4px' }}>Target Scene</label>
                            <select
                                value={newHotspot.target_scene}
                                onChange={(e) => setNewHotspot({ ...newHotspot, target_scene: e.target.value })}
                                style={{ width: '100%', padding: '8px', borderRadius: '4px', border: '1px solid #ccc', fontSize: '14px' }}
                            >
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', marginBottom: '8px' }}>
                            <div>
                                <label style={{ display: 'block', fontSize: '12px', marginBottom: '4px' }}>Yaw (°)</label>
                                <input
                                    type="number"
                                    placeholder="0"
                            ...
                            <div>
                                <label style={{ display: 'block', fontSize: '12px', marginBottom: '4px' }}>Pitch (°)</label>
                                <input
                                    type="number"
                                    placeholder="0"
                        ...
                        <button
                            type="submit"
                            style={{ width: '100%', padding: '8px', background: theme.colors.primary, color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '14px', fontWeight: '600' }}
                        >
                            Create Hotspot
                        </button>
                    </form>
