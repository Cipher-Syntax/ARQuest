                ...
                <tr key={user.id} className="hover:bg-brand-light/30 transition-colors">
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      ...
                      <p className="font-bold text-gray-900 text-sm">{user.first_name || user.last_name ? `${user.first_name} ${user.last_name}` : user.username}</p>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4">
                    <Badge variant={user.role === 'admin' ? 'brand' : user.role === 'professional' ? 'warning' : 'gray'}>
                      {user.role.charAt(0).toUpperCase() + user.role.slice(1)}
                    </Badge>
                  </td>
                  <td className="px-6 py-4 text-right">
                    <div className="flex items-center justify-end gap-2">
                      <div className={`w-2 h-2 rounded-full ${user.is_active ? 'bg-green-500 animate-pulse' : 'bg-gray-300'}`} />
