...
export default function QuestsPage({ hideHeader }) {
  ...
  const filteredQuests = quests.filter(q => 
    ...
    (q.target_building_name && q.target_building_name.toLowerCase().includes(searchTerm.toLowerCase()))
  );
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {filteredQuests.map(quest => (
          <Card key={quest.id} className="group hover:border-brand transition-colors flex flex-col h-full">
            ...
            <p className="text-xs font-semibold text-gray-500 mb-2">📍 {quest.target_building_name || 'Building'}</p>
            ...
            <div className="mt-auto pt-4 border-t border-brand-border flex items-center justify-between">
              <span className="text-xs font-bold text-gray-600">Reward: <span className="text-brand">{quest.reward_points} XP</span></span>
              <div className="flex gap-2">
                <Button variant="ghost" size="xs" onClick={() => handleOpenEditModal(quest)}>
                  <Edit3 size={14} />
                </Button>
                <Button variant="ghost" size="xs" className="text-red-600 hover:text-red-700 hover:bg-red-50" onClick={() => handleDeleteClick(quest.id)}>
                  <Trash2 size={14} />
                </Button>
              </div>
            </div>
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-gray-900/40 backdrop-blur-sm">
          <div className="bg-white rounded-lg shadow-2xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in duration-200">
            <div className="px-6 py-4 border-b border-brand-border flex items-center justify-between">
              <h3 className="font-bold text-gray-900">{editingQuest ? 'Edit Quest' : 'Create Quest'}</h3>
            ...
            <div className="p-6 space-y-5">
              <div className="space-y-2">
                <label className="text-xs font-bold text-gray-500 uppercase tracking-wider">Quest Title</label>
                <input
                  type="text"
                  value={newTitle}
              ...
              <div className="space-y-2">
                <label className="text-xs font-bold text-gray-500 uppercase tracking-wider">Target Building</label>
                <select
                  value={newBuildingId}
              ...
              <div className="space-y-2">
                <label className="text-xs font-bold text-gray-500 uppercase tracking-wider">Reward (XP)</label>
                <input
                  type="number"
                  value={newReward}
              ...
              <div className="space-y-2">
                <label className="text-xs font-bold text-gray-500 uppercase tracking-wider">Hint / Description</label>
                <textarea
                  value={newHint}
              ...
              <label className="flex items-center gap-3 cursor-pointer">
                <input type="checkbox" checked={isActive} onChange={(e) => setIsActive(e.target.checked)} className="rounded border-gray-300 text-brand focus:ring-brand" />
                <span className="text-sm font-bold text-gray-700">Active (Visible to players)</span>
              </label>
            </div>
