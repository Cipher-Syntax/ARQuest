        ...
        <Card className="lg:col-span-2">
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-xs text-gray-500 mt-0.5">Building unlocks recorded this week</p>
            </div>
          ...
          <div className="h-48 flex items-end justify-between gap-3 px-2">
            {WEEKLY_DATA.map((data, i) => (
              <div key={i} className="flex-1 flex flex-col items-center gap-3 group">
                ...
                <span className="text-[11px] font-bold text-gray-400 uppercase">{data.day}</span>
              </div>
            ))}
          </div>
        </Card>
            ...
            {BUILDING_STATUS.map((b, i) => (
              <div key={i} className="flex items-center justify-between py-2.5 border-b border-brand-border last:border-0">
                ...
                <Badge variant={b.status === 'Live' ? 'success' : 'warning'}>
                  {b.status}
                  {b.status === 'Live' && <ArrowUpRight size={10} className="ml-1 inline" />}
                </Badge>
              </div>
            ))}
          </div>
