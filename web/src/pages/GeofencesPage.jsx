        ...
        try {
            if (editingGeo) {
                ...
                if (updatedGeofenceId) {
                    ...
                    const newGeo = await buildingService.createGeofence(
                        editingGeo.id,
                        geoPayload,
                    );
    ...
    const handleConfirmDelete = async () => {
        if (geoToDelete) {
            try {
                ...
                setGeofences((prev) =>
                    prev.filter((g) => g.id !== geoToDelete),
                );
                setGeoToDelete(null);
                        ...
                        {!selectedGeoId && (
                            <div className="flex flex-col items-center justify-center h-full bg-gray-50 rounded-lg border-2 border-dashed border-gray-200 p-8 text-center">
                                ...
                                <p className="text-gray-500 text-sm">
                                    Click on any map marker to view and manage
                                    its geofence boundary details.
                                </p>
                            </div>
                        )}
