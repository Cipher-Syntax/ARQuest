				...
				>
					...
					>
						<div className="border-b border-gray-100 pb-4">
							<h2 className="text-xl font-bold text-gray-800">Building Information</h2>
							<p className="text-sm text-gray-500 mt-1">Update the core details and metadata for this building.</p>
						</div>
						...
						<div className="grid grid-cols-2 gap-4">
							<div>
								<label className="block text-sm font-semibold text-gray-700 mb-1.5">
									Latitude {building.status !== 'DRAFT' && <span className="text-red-500">*</span>}
								</label>
								<input
									type="number"
									name="latitude"
								...
								/>
								{errors.latitude && <div className="text-red-500 text-xs mt-1.5 font-medium">{errors.latitude}</div>}
							</div>
							<div>
								<label className="block text-sm font-semibold text-gray-700 mb-1.5">
									Longitude {building.status !== 'DRAFT' && <span className="text-red-500">*</span>}
								</label>
								<input
									type="number"
									name="longitude"
								...
								/>
								{errors.longitude && <div className="text-red-500 text-xs mt-1.5 font-medium">{errors.longitude}</div>}
							</div>
						</div>

						{/* Status & Active */}
						<div className="flex flex-wrap items-center gap-8 bg-gray-50 p-4 border border-gray-100" style={{ borderRadius: theme.radius.sm }}>
							<div className="flex flex-col gap-1.5 w-full sm:w-auto flex-1">
								<label className="text-sm font-semibold text-gray-700">Publishing Status</label>
								<select
									name="status"
							...
							<label className="flex items-center gap-3 text-sm font-medium text-gray-700 cursor-pointer mt-1 sm:mt-5 bg-white px-4 py-2 border border-gray-200 shadow-sm hover:bg-gray-50 transition-colors" style={{ borderRadius: theme.radius.sm }}>
								<input
									type="checkbox"
									name="is_active"
						...
						<div className="space-y-5 pt-2">
							<div>
								...
								<div className="relative" ref={deptDropdownRef}>
									...
									{deptDropdownOpen && (
										<div className="absolute top-full left-0 right-0 mt-2 bg-white border border-gray-100 shadow-lg z-50 max-h-64 flex flex-col overflow-hidden" style={{ borderRadius: theme.radius.sm }}>
											...
											<div className="overflow-y-auto">
												...
												{departments.filter(d => d.name.toLowerCase().includes(deptSearch.toLowerCase()) || d.code.toLowerCase().includes(deptSearch.toLowerCase())).map(dept => (
													...
													>
														<div className={building.primary_department_id === dept.id ? 'font-semibold' : 'font-medium'}>{dept.name}</div>
														<div className={`text-xs mt-0.5 ${building.primary_department_id === dept.id ? 'text-[#8a1538]/80' : 'text-gray-400'}`}>Code: {dept.code}</div>
													</div>
												))}
											</div>
										</div>
							...
							<div>
								...
								<div className="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-48 overflow-y-auto p-3 bg-gray-50 border border-gray-200 shadow-inner" style={{ borderRadius: theme.radius.sm }}>
									{departments.map(dept => (
										<label key={dept.id} className="flex items-start gap-2.5 text-sm cursor-pointer p-2 hover:bg-white rounded transition-colors border border-transparent hover:border-gray-200 hover:shadow-sm">
											<input
												type="checkbox"
											...
											/>
											<span className="text-gray-700 leading-tight">{dept.name}</span>
										</label>
									))}
									{departments.length === 0 && <span className="text-sm text-gray-400 p-2">No colleges found</span>}
								</div>
							</div>
						</div>
						...
						<div className="pt-6 border-t border-gray-100 flex flex-col gap-5">
							<div className="flex items-center justify-between">
								<div>
									<h2 className="text-lg font-bold text-gray-800">3D Model Configuration</h2>
									<p className="text-xs text-gray-500 mt-1">Upload a GLTF/GLB file to enable the 3D Viewer.</p>
								</div>
								<label className="flex items-center gap-2 text-sm font-medium text-gray-700 cursor-pointer bg-gray-50 px-3 py-1.5 border border-gray-200 shadow-sm hover:bg-gray-100 transition-colors" style={{ borderRadius: theme.radius.sm }}>
									<input
										type="checkbox"
										name="model_active"
							...
							<div className="flex flex-col gap-4">
								...
								{building.model_url && !(building.model_file instanceof File) && (
									<div className="flex flex-col gap-3 mt-1 px-1">
										<span className="text-xs font-semibold text-gray-600">Current model uploaded. Drop a new file above to replace it.</span>
										<button
											type="button"
