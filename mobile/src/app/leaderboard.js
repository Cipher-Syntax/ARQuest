            ...
            <View>
                ...
                <View style={styles.podiumContainer}>
                    {podiumOrder.map((student, index) => {
                        if (!student) {
                            return <View key={`empty-${index}`} style={styles.podiumItem} />;
                        }
                        ...
                        return (
                            <View key={student.username} style={styles.podiumItem}>
                                <Text style={styles.podiumUsername} numberOfLines={1}>{student.username}</Text>
                                <Text style={styles.podiumPoints}>{student.points} pts</Text>
                                <View style={[styles.podiumBlock, { height, backgroundColor: color }]}>
                                    <Text style={styles.podiumRank}>{rankStr}</Text>
                                    {isFirst && <Trophy color="#000" size={32} style={{marginTop: 10}}/>}
                                </View>
                            </View>
                        );
                    })}
                        ...
                        {recentActivity.map((activity, idx) => (
                            <View key={idx} style={styles.recentItem}>
                                ...
                                <View style={styles.recentContent}>
                                    <Text style={styles.recentUser}><Text style={{fontWeight: 'bold'}}>@{activity.username}</Text> completed</Text>
                                    <Text style={styles.recentQuest}>"{activity.quest_title}" at {activity.building_name}</Text>
                                    <Text style={styles.recentPoints}>+{activity.points} Points</Text>
                                </View>
                            </View>
                        ))}
                    </View>
                ) : (
                    <Text style={styles.emptyTextSmall}>No quests completed yet today.</Text>
                )}

                {/* --- OTHER TOP EXPLORERS --- */}
                <View style={[styles.sectionHeader, { marginTop: 30, marginBottom: 15 }]}>
                    <Trophy size={20} color={theme.colors.textSecondary} />
                    <Text style={[styles.sectionTitle, { color: theme.colors.textSecondary }]}>OTHER TOP EXPLORERS</Text>
                </View>
                {leaderboard.length <= 3 && (
                    <Text style={styles.emptyTextSmall}>No other explorers have joined the ranks yet.</Text>
                )}
            </View>
        );
    };
        ...
        <SafeAreaView style={styles.container} edges={['top']}>
            ...
            {loading ? (
                ...
                <FlatList
                    ...
                    ListEmptyComponent={<Text style={{textAlign: 'center', marginTop: 20, color: theme.colors.textMuted}}>No rankings available.</Text>}
                />
            )}
        </SafeAreaView>
    );
