...
export default function AR3DModelOverlay({ modelUrl, buildingName, capturing, onSnapshotReady, style }) {
    ...
    useEffect(() => {
        if (capturing && webViewRef.current) {
            webViewRef.current.postMessage(JSON.stringify({ type: 'request_snapshot' }));
        } else if (!capturing) {
    ...
    return (
        <View style={[styles.container, style]} pointerEvents="none">
            ...
            />
            {snapshotData && (
                <Image source={{ uri: snapshotData }} style={[styles.webview, StyleSheet.absoluteFillObject]} resizeMode="contain" />
            )}
            {loading && (
